            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ${projects.items.map(project => `
                    <div class="project-card h-full flex flex-col border border-gray-200 dark:border-gray-800 rounded-xl p-6 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm transition-all duration-300">
                        <h3 class="font-semibold mb-3 text-lg">${escapeHtml(project.title)}</h3>
                        ...
                        <div class="flex gap-6 mt-auto">
                            ${project.links.live && isSafeUrl(project.links.live) ? `
                                <a href="${project.links.live}" class="text-accent hover:text-amber-700 font-medium text-sm transition-colors">live →</a>
                            ` : ''}
                            ${project.links.github && isSafeUrl(project.links.github) ? `
                                <a href="${project.links.github}" class="text-accent hover:text-amber-700 font-medium text-sm transition-colors">github →</a>
                            ` : ''}
                        </div>
                    </div>
                `).join('')}
                            ...
                            <div class="flex-1">
                                <div class="flex items-baseline gap-2 mb-2">
                                    ...
                                    <div class="flex gap-2">
                                        ${project.links.live && isSafeUrl(project.links.live) ? `
                                            <a href="${project.links.live}" class="text-accent hover:text-amber-700 text-xs font-medium transition-colors">live →</a>
                                        ` : ''}
                                        ${project.links.github && isSafeUrl(project.links.github) ? `
                                            <a href="${project.links.github}" class="text-accent hover:text-amber-700 text-xs font-medium transition-colors">github →</a>
                                        ` : ''}
                                    </div>
                                </div>
        ...
        if (achievementsContainer) {
            const achievementsHtml = Array.isArray(achievements.achievements) && achievements.achievements.length > 0
                ? achievements.achievements.map(achievement => {
                    ...
                    const safeDescription = achievement.description || '';
...
function populateFooter(footer) {
    ...
    if (footerElement) {
        footerElement.innerHTML = `
            ...
            <p class="text-gray-600 dark:text-gray-400 text-sm">${footer.inspired}</p>
